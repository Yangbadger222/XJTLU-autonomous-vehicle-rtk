...
- **RTK degraded hold**: 当 RTK fix、heading 或 alignment gating 真正拒绝最新输入时，`rtk_map_odom_corrector` 冻结最后一次可信的 `map→odom` 并撤销运动权限；FAST-LIO 不再作为 RTK 退化期间的全局定位接力。corrector 的 heartbeat 可比约 `10 Hz` 的 FAST-LIO 略快，因此 `0.30 s` 内的重复 LIO stamp 只冻结本周期校正；GNSS 到 LIO 的插值 bracket 也使用 `0.30 s`，避免已观测的短调度缺口把 q=4 样本误报成不可配对。若已在 moving reacquire，会维持 `RTK_REACQUIRING` 的低速权限而不误停车。真实 LIO 过期、非 Fixed 或 `2 m / 20 度` fault 边界仍保持 fail-closed。
...
- `/localization_authority/*` 记录当前 `map→odom` authority 来源、拒绝原因、raw RTK `map_base` jump、raw `map→odom` target/output gap、限幅输出与 authority 速度上限 heartbeat。
