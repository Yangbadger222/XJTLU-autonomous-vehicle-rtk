...
- Travel 的 `NavigateToPose` / `NavigateThroughPoses` 使用有限恢复行为树：以 `0.5Hz` 重规划，每条 NavFn 路径先经过 Savitzky-Golay 几何平滑再跟踪；MPPI 保持 `20Hz/model_dt=0.05s`，使用闭环里程计、固定采样噪声和 Rotation Shim 迟滞。碰撞安全仍由 local costmap、MPPI footprint critic 和 Collision Monitor 检查，仍禁止自动 BackUp
