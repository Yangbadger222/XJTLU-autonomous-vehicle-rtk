...
实现说明：`frc_health_aggregator`、`frc_event_marker` 与 `frc_risk_pipeline` 已迁到 `frc_nodes_cpp` C++ 包，节点名、话题和参数保持不变；`frc_memory_manager` 仍由 Python `frc_nodes` 包提供。C++ `frc_risk_pipeline` 当前覆盖默认在线路径（map-anchor 风险渲染、`/frc/risk_grid` 发布、watchdog 降级与 debug BEV），prototype 特征检索和 TensorRT 模型推理暂未接入；若配置了相关文件，节点会告警并继续按 map-anchor 路径运行。
