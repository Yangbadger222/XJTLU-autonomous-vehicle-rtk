...
9. Travel 是独立的室内地图包 profile。global costmap 只用静态地图/膨胀，local costmap 读取 `/fastlio2/body_cloud_nav2`。室内验收档为 `vx_max=0.30m/s`、`vx_std=0.16`、`ax_max=0.40m/s2`、`20Hz`、`batch_size=200`、`time_steps=40`、`regenerate_noises=true`。2026-07-12 实车运行中 `batch_size=500` 和后续 300 都持续错过 20Hz，300 档还在约 5 秒后报 `Optimizer fail to compute path`，因此进一步降到 `200x40` 并把短时优化失败容忍从 `0.6s` 放宽到 `2.0s`；`model_dt=0.05s` 不变，不能通过降低控制频率规避。Travel 用 Rotation Shim 包装 MPPI，路径方向误差超过 `0.35rad` 时以 `0.30rad/s` 原地对准；`VelocityDeadbandCritic` 以 `[0.14, 0.0, 0.18]` 避开实车不可靠起步区。真实 `650x500mm` 外廓加每侧 `25mm` 仍作为规划 footprint；障碍云使用 `0.80x0.60m` 自滤框和实测窄三维补丁覆盖本体回波，Collision Monitor Stop 框保持 `0.80x0.64m`。侧向扫掠安全由 local costmap 的真实 footprint 继续检查。
10. Travel 使用有限恢复行为树：1Hz 持续重规划，规划失败清全局代价，控制失败清局部代价，外层最多 5 次按 Wait 1s、碰撞检查 Spin 90 度、清局部/全局代价轮询恢复；不执行 BackUp。2026-07-12 实车中，运动约 17 秒后连续 ICP 撤掉 `map->odom`，造成整条控制链 TF extrapolation 并取消目标；因此当前 Travel 只在初始位姿时执行 PCD ICP，接受后固定 `map->odom` 偏移，由 FAST-LIO2 高频 `odom->base_footprint` 传播运动。`map->odom` 仍带 `0.10s` 未来容差；长时间 LIO 漂移验收前必须修复并重新验证运行时地图匹配。实测 100 条底盘状态中出现单帧非法 `ctrl_mode=4/252` 和 `ps2_key=255/252`，适配器对底盘模式使用 `1.0s` 去抖。Collision Monitor Stop/Slow 的 `max_points` 均为 `10`。
11. `20-37-08` Travel rosbag 证明原始 NavFn 路径是轨迹扭动主因：首条 `4.23m` 路径累计折转 `15.23rad`、含 30 个明显折角，车辆 `74.6s` 内行驶 `5.73m` 但净位移只有 `3.86m`，角速度换向 46–52 次。Travel 现在每次重规划后执行 Savitzky-Golay `SmoothPath`；`20-58-24` 包进一步证明 smoother 的全局 footprint 碰撞复检会把所有路径拒绝，因此该重复检查关闭，运行安全继续由 local costmap、MPPI footprint critic 和 Collision Monitor 保证。local costmap 从 `15Hz` 降到与障碍云一致的 `10Hz`。同一包还显示车辆进入 `0.35m` 到点半径后持续收到 `0.04rad/s`，低于实车有效转动区，因此定位速度门只在纯原地、非零转向时补到 `0.20rad/s`；零命令、线性行驶和安全停车不补偿。FAST-LIO2 的 `odom->base_footprint` TF 增加 `0.05s` 前向容差。
