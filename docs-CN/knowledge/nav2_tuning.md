...
11. `20-37-08` 包证明原始 NavFn 路径锯齿，Travel 因此启用 Savitzky-Golay `SmoothPath`；`20-58-24` 包证明 smoother footprint 复检会误拒绝全部路径，因此重复检查关闭。`21-06-57` 包第一段成功且轨迹效率从 `0.67` 提升到 `0.82`，安全角速度换向从 46 降到 24；第二段被真实障碍持续触发 Slow 后，输出落到 `vx=0.04–0.07m/s,wz≈0.20rad/s` 并卡入底盘死区。死区处理现移到 Collision Monitor 后：低速转弯改为零线速度、至少 `0.20rad/s` 原地转向，Stop 与零命令保持不变。FAST-LIO2 TF 保持 `0.05s` 前向容差。
