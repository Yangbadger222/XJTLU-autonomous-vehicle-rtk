...
9. Travel 是独立的室内地图包 profile。global costmap 只用静态地图/膨胀，local costmap 读取 `/fastlio2/body_cloud_nav2`。实车首轮发现旧 35% Slow 区会把命令压到约 `0.02-0.034m/s`；调整 Slow 区后 MPPI 又长期顶到 `0.45m/s`。室内验收档最终回调为 `vx_max=0.30m/s`、`vx_std=0.16`、`ax_max=0.40m/s2`、`20Hz`、`batch_size=500`、`regenerate_noises=true`。Travel 用 Rotation Shim 包装 MPPI：路径方向误差超过 `0.35rad` 时以 `0.30rad/s` 原地对准，随后才交给 MPPI；`VelocityDeadbandCritic` 以 `[0.14, 0.0, 0.18]` 惩罚低于实车可靠起步区间的轨迹，避免同时下发很小的线/角速度使单侧车轮被 PID 锁在静摩擦区。串口前另有 `0.30m/s2` 和 `0.80rad/s2` 的末级加速恢复限制，零速和降速不受延迟。真实外廓仍按 `650x500mm` 加每侧 `25mm` 配为 polygon，`CostCritic.consider_footprint=true`，终点 yaw 容差为 `0.20rad`。
