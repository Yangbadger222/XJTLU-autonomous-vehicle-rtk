...
9. Travel 是独立的室内地图包 profile。global costmap 只用静态地图/膨胀，local costmap 读取 `/fastlio2/body_cloud_nav2`。室内验收档保持 `20Hz/model_dt=0.05s`、`batch_size=200`、`time_steps=40`，不能通过降低控制频率规避负载。`21-06-57` 包仍显示频繁左右换向，因此改为 `open_loop=false`、`regenerate_noises=false`、`wz_std=0.10`，Rotation Shim 使用 `angular_dist_threshold=0.35`、`angular_disengage_threshold=0.15` 与 `closed_loop=true`。真实 footprint、障碍云自滤和 Collision Monitor 几何保持不变。
