...
10. Travel 使用 fail-stop 行为树，不提供自动 Spin、BackUp 或清图恢复。定位以 1Hz 连续 ICP、`overlap>=0.35`、最大单次修正 `0.30m/0.20rad` 和 `alpha=0.15` 平滑；`map->odom` 带 `0.10s` 未来容差。连续 ICP 失败阈值为 8 次，DEGRADED 最近可信 TF 宽限为 `5.0s`。活动目标遇到定位短时不可用时，速度门立即输出零速，但适配器等待 `5.0s` 恢复后才取消目标；持续 LOST 仍 fail-stop。实测 100 条底盘状态中出现单帧非法 `ctrl_mode=4/252` 和 `ps2_key=255/252`，因此适配器对底盘模式使用 `1.0s` 去抖，持续退出 HOST 才取消目标。Collision Monitor Stop 区为 `0.80x0.64m`，Slow 区为前方 `x=0.45~0.85m, y=+-0.30m`、60% 限速。宽路静态实测 Stop 区散点为每帧 `0~6`、平均 `2.39`，因此 Stop/Slow 的 `max_points` 均设为 `10`；真实平面障碍形成的密集点簇仍会触发。
