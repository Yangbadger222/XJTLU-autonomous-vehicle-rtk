...
9. Travel 是独立的室内地图包 profile。global costmap 只用静态地图/膨胀，local costmap 读取 `/fastlio2/body_cloud_nav2`。室内验收档为 `vx_max=0.30m/s`、`vx_std=0.16`、`ax_max=0.40m/s2`、`20Hz`、`batch_size=300`、`regenerate_noises=true`。2026-07-12 实车运行中 `batch_size=500` 的控制循环持续错过 20Hz，因此降到 300 并关闭 critics stats；`model_dt=0.05s` 不变，不能通过降低控制频率规避。Travel 用 Rotation Shim 包装 MPPI，路径方向误差超过 `0.35rad` 时以 `0.30rad/s` 原地对准；`VelocityDeadbandCritic` 以 `[0.14, 0.0, 0.18]` 避开实车不可靠起步区。真实 `650x500mm` 外廓加每侧 `25mm` 仍作为规划 footprint；障碍云使用 `0.80x0.60m` 自滤框覆盖实测本体回波，Collision Monitor Stop 框保持 `0.80x0.64m`。实测更宽的 `0.90x0.70m` Stop 框会把 `y=-0.33~-0.35m` 的可通行侧向点反复判成停车，侧向扫掠安全由 local costmap 的真实 footprint 继续检查。
10. Travel 使用 fail-stop 行为树，不提供自动 Spin、BackUp 或清图恢复。定位以 1Hz 连续 ICP、`overlap>=0.35`、最大单次修正 `0.30m/0.20rad` 和 `alpha=0.15` 平滑；`map->odom` 带 `0.10s` 未来容差。一次 ICP 失败可在 DEGRADED 中使用最近可信 TF 最多 `2.5s`。活动目标遇到定位短时不可用时，速度门立即输出零速，但适配器等待 `2.5s` 恢复后才取消目标；持续 LOST 仍 fail-stop。Collision Monitor Stop 区为 `0.80x0.64m`，Slow 区为前方 `x=0.45~0.85m, y=+-0.30m`、60% 限速。宽路静态实测 Stop 区散点为每帧 `0~6`、平均 `2.39`，因此 Stop/Slow 的 `max_points` 均设为 `10`；真实平面障碍形成的密集点簇仍会触发。
