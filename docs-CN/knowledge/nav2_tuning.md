...
10. Travel 使用 fail-stop 行为树，不提供自动 Spin、BackUp 或清图恢复。定位以 1Hz 连续 ICP、`overlap>=0.35`、最大单次修正 `0.30m/0.20rad` 和 `alpha=0.15` 平滑；`map->odom` 带 `0.10s` 未来容差。一次 ICP 失败可在 DEGRADED 中使用最近可信 TF 最多 `2.5s`，避免瞬态失败立即取消；超过窗口、输入过期或进入 LOST 仍强制停车。Collision Monitor Stop 区为 `0.80x0.64m`，Slow 区为前方 `x=0.45~0.85m, y=+-0.30m`、60% 限速。
