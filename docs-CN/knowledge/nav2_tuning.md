...
Corridor 现已拆分 local motion、global correction 与 command authority。`rtk_map_odom_corrector` 使用 2 秒 `/fastlio2/lio_odom` 时间戳历史对齐 RTK 观测，要求 5 个一致的 Fixed 样本，并在 `map→base_footprint` 空间以不超过 `0.20 m/s`、`2 deg/s` 慢释放。NORMAL release 最多连续 10 个样本顶住速率上限，随后进入 backlog hold。中等 backlog（`0.50-2.0 m` 或 `5-20 deg`）需连续停车 1 秒后才慢释放；更大 backlog 锁存 `FAULT_HOLD`。
