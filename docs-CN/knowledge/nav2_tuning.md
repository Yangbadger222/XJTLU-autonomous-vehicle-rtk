...
9. Travel 是独立的室内地图包 profile。MPPI 必须保持 `20Hz/model_dt=0.05s`，不能单独降到 15Hz。最新 `22-03-41` 包在约 110.5 秒活动目标中漏 58 个周期，且 Humble CostCritic 对全部 `batch_size*time_steps` 点串行评分，因此 `time_steps` 从 40 降到 32、`batch_size` 保持 160，每周期候选点从 6400 降为 5120；移除 `vx_min=0` 下冗余的 PreferForwardCritic 和 Humble 未声明的 CostCritic `trajectory_point_step`。local costmap 内部仍以 10Hz 更新，但发布降到 2Hz；静态 global costmap 更新/发布降到 1Hz，减少 rosbag/序列化开销。
10. Rotation Shim 现在只处理 `>0.65rad` 的大初始偏差，`0.35rad` 后交还 MPPI，且不处理终点朝向。VelocityDeadbandCritic 从权重 35、`[0.14,0,0.18]` 降到 15、`[0.12,0,0.08]`；PathAlign 从 12 降到 6、PathFollow 从 16 降到 12。后处理器不再无条件把纯旋转补到 `0.20rad/s`，只在 Collision Monitor 确认 Slow 且混合线速度落入底盘死区时清除平移并原样保留角速度。这直接撤销 `22-03-41` 包中后处理改写 55.9% 命令、4433 条变成纯旋转的过度干预。
11. Travel 以 1Hz 重规划。FollowPath 失败先清 local costmap、立即重新规划和平滑；外层再次先做同样动作，只有 `IsStuck` 成立才执行 `0.52rad` 短 Spin，然后才 Wait/清双图，仍不 BackUp。`prior_map_tf_authority` 是唯一 `map->odom` owner：localizer 提供初始种子，AMCL 提供运行中候选，authority 用协方差、时间差、目标跳变、TF 单步和 `map->base` 等效跳变门限平滑释放，解决此前走廊末段约 1m 的长期漂移且避免 TF 突跳。
