...
9. Travel 是独立的室内地图包 profile。global costmap 只用静态地图/膨胀，local costmap 读取 `/fastlio2/body_cloud_nav2`。室内验收档为 `vx_max=0.30m/s`、`vx_std=0.16`、`ax_max=0.40m/s2`、`20Hz`、`batch_size=300`、`regenerate_noises=true`。2026-07-12 实车运行中 `batch_size=500` 的控制循环持续错过 20Hz，因此降到 300 并关闭 critics stats；`model_dt=0.05s` 不变，不能通过降低控制频率规避。Travel 用 Rotation Shim 包装 MPPI，路径方向误差超过 `0.35rad` 时以 `0.30rad/s` 原地对准；`VelocityDeadbandCritic` 以 `[0.14, 0.0, 0.18]` 避开实车不可靠起步区。真实 `650x500mm` 外廓加每侧 `25mm` 仍作为规划 footprint；障碍云自滤波扩大到 `0.80x0.60m` 以覆盖实测本体回波，Collision Monitor Stop 框外扩到 `0.90x0.70m`，保留自滤框外真实停车带。
10. Travel 使用 fail-stop 行为树，不提供自动 Spin、BackUp 或清图恢复。定位以 1Hz 连续 ICP、`overlap>=0.35`、最大单次修正 `0.30m/0.20rad` 和 `alpha=0.15` 平滑；`map->odom` 带 `0.10s` 未来容差。一次 ICP 失败可在 DEGRADED 中使用最近可信 TF 最多 `2.5s`，避免瞬态失败立即取消；超过窗口、输入过期或进入 LOST 仍强制停车。Collision Monitor Stop 区为 `0.90x0.70m`，Slow 区为前方 `x=0.45~0.85m, y=+-0.30m`、60% 限速。
