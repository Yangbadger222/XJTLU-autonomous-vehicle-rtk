...
9. Travel 是独立的室内地图包 profile。global costmap 只用静态地图/膨胀，local costmap 读取 `/fastlio2/body_cloud_nav2`。室内验收档保持 `20Hz/model_dt=0.05s`、`batch_size=200`、`time_steps=40`，不能通过降低控制频率规避负载。`21-06-57` 包仍显示频繁左右换向，因此改为 `regenerate_noises=false`、`wz_std=0.10`，Rotation Shim 使用 `angular_dist_threshold=0.35`、`angular_disengage_threshold=0.15` 与 `closed_loop=true`。Humble MPPI 不声明 `open_loop` 参数，配置中不保留无效键。真实 footprint、障碍云自滤和 Collision Monitor 几何保持不变。
...
11. `20-37-08` 包证明原始 NavFn 路径锯齿，Travel 因此启用 Savitzky-Golay `SmoothPath`；`20-58-24` 包证明 smoother footprint 复检会误拒绝全部路径，因此重复检查关闭。`21-06-57` 包第一段成功且轨迹效率从 `0.67` 提升到 `0.82`，安全角速度换向从 46 降到 24；第二段被真实障碍持续触发 Slow 后，输出落到 `vx=0.04–0.07m/s,wz≈0.20rad/s` 并卡入底盘死区。死区处理现移到 Collision Monitor 后：低速转弯改为零线速度、至少 `0.20rad/s` 原地转向，Stop 与零命令保持不变。global inflation 从小于 footprint 内切半径的 `0.25m` 修正为 `0.35m`，避免路径先天贴障碍；FAST-LIO2 TF 保持 `0.05s` 前向容差。
