...
10. Travel 使用有限恢复行为树：1Hz 持续重规划，规划失败清全局代价，控制失败清局部代价，外层最多 5 次按 Wait 1s、碰撞检查 Spin 90 度、清局部/全局代价轮询恢复；不执行 BackUp 或路径平滑。2026-07-12 实车中，运动约 17 秒后连续 ICP 撤掉 `map->odom`，造成整条控制链 TF extrapolation 并取消目标；因此当前 Travel 只在初始位姿时执行 PCD ICP，接受后固定 `map->odom` 偏移，由 FAST-LIO2 高频 `odom->base_footprint` 传播运动。`map->odom` 仍带 `0.10s` 未来容差；长时间 LIO 漂移验收前必须修复并重新验证运行时地图匹配。实测 100 条底盘状态中出现单帧非法 `ctrl_mode=4/252` 和 `ps2_key=255/252`，适配器对底盘模式使用 `1.0s` 去抖。Collision Monitor Stop/Slow 的 `max_points` 均为 `10`。
