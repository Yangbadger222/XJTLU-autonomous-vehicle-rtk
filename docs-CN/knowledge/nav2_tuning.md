...
9. Travel 是独立的室内地图包 profile。global costmap 只用静态地图/膨胀，local costmap 读取 `/fastlio2/body_cloud_nav2`。室内验收档保持 `20Hz/model_dt=0.05s`、`time_steps=40`，不能只降低控制频率而不同时修改模型步长。`21-45-44` 包记录到 146 次 20Hz 周期超时，因此在保留 2 秒预测时域时把 `batch_size` 从 200 降到 160。`21-06-57` 包仍显示频繁左右换向，因此使用 `regenerate_noises=false`、`wz_std=0.10`，Rotation Shim 使用 `angular_dist_threshold=0.35`、`angular_disengage_threshold=0.15` 与 `closed_loop=true`。Humble MPPI 不声明 `open_loop` 参数，配置中不保留无效键。真实 footprint、障碍云自滤和 Collision Monitor 几何保持不变。
10. Travel 使用有限恢复行为树：`0.5Hz` 持续重规划，规划失败清全局代价，控制失败清局部代价，外层最多 5 次按 Wait、碰撞检查 Spin、清代价地图轮询恢复；不执行 BackUp。定位只在初始位姿时执行 PCD ICP，接受后固定 `map->odom` 偏移，由 FAST-LIO2 传播运动。底盘模式保持 `1.0s` 去抖，Collision Monitor Stop/Slow 的 `max_points` 均为 `10`。
11. `20-37-08` 包证明原始 NavFn 路径锯齿，Travel 因此启用 Savitzky-Golay `SmoothPath`；`20-58-24` 包证明 smoother footprint 复检会误拒绝全部路径，因此重复检查关闭。`21-06-57` 包第一段成功且轨迹效率从 `0.67` 提升到 `0.82`；第二段被真实障碍持续触发 Slow 后卡入底盘死区。`21-45-44` 包进一步显示 Collision Monitor 只改写 2/2260 条命令，真正停滞发生在 30 秒和 10 秒的纯旋转阶段：Nav2 持续输出约 `0.04rad/s`，LIO 位移与转角均接近零，随后平移型进度检查三次失败。因此所有非零纯旋转命令无条件补到至少 `0.20rad/s`，Slow 混合低速转弯仍只在确认减速后转为原地转向；进度插件改为 `PoseProgressChecker`，`0.15rad` 转向可重置进度窗口。global inflation 使用仍高于 `0.285m` 内切半径的 `0.30m` 最小安全档；FAST-LIO2 TF 保持 `0.05s` 前向容差。
