...
11. Travel 以 1Hz 重规划。FollowPath 失败先清 local costmap、立即重新规划和平滑；外层再次先做同样动作，只有 `IsStuck` 成立才执行 `0.52rad` 短 Spin，然后才 Wait/清双图，仍不 BackUp。`prior_map_tf_authority` 是唯一 `map->odom` owner：除协方差、时间差和跳变门限外，AMCL 还必须通过 5 帧/至少 3 帧短窗一致性检查；稳定中值最多每秒释放一次，平移/yaw 使用独立死区。两个最新包离线回放中 TF 改值从 `31/35` 次降至 `7/6` 次，累计 yaw 从约 `5.1deg` 降至 `0.31/1.20deg`，同时仍释放约 `5~6cm` 的稳定平移校正。
