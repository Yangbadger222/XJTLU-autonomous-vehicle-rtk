...
9. Travel 是独立的室内地图包 profile。global costmap 只用静态地图/膨胀，local costmap 读取 `/fastlio2/body_cloud_nav2`。实车首轮发现旧 35% Slow 区会把命令压到约 `0.02-0.034m/s`；调整 Slow 区后 MPPI 又长期顶到 `0.45m/s`。室内验收档最终回调为 `vx_max=0.30m/s`、`vx_std=0.16`、`ax_max=0.40m/s2`、`20Hz`、`batch_size=500`、`regenerate_noises=true`。串口前另有 `0.30m/s2` 和 `0.80rad/s2` 的末级加速恢复限制，零速和降速不受延迟。真实外廓仍按 `650x500mm` 加每侧 `25mm` 配为 polygon，`CostCritic.consider_footprint=true`，终点 yaw 容差为 `0.20rad`。
