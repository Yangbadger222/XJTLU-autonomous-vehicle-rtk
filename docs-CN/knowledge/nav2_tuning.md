...
11. `20-37-08` Travel rosbag 证明原始 NavFn 路径是轨迹扭动主因：首条 `4.23m` 路径累计折转 `15.23rad`、含 30 个明显折角，车辆 `74.6s` 内行驶 `5.73m` 但净位移只有 `3.86m`，角速度换向 46–52 次。Travel 现在每次重规划后执行带碰撞检查的 Savitzky-Golay `SmoothPath`；local costmap 从 `15Hz` 降到与障碍云一致的 `10Hz`。同一包还显示车辆进入 `0.35m` 到点半径后持续收到 `0.04rad/s`，低于实车有效转动区，因此定位速度门只在纯原地、非零转向时补到 `0.20rad/s`；零命令、线性行驶和安全停车不补偿。FAST-LIO2 的 `odom->base_footprint` TF 增加 `0.05s` 前向容差，覆盖 10Hz LIO 与 20Hz controller 的调度差。
