...
10. Travel 使用有限恢复行为树：1Hz 持续重规划，规划失败清全局代价，控制失败清局部代价，外层最多 5 次按 Wait 1s、碰撞检查 Spin 90 度、清局部/全局代价轮询恢复；不执行 BackUp 或路径平滑。定位以 1Hz 连续 ICP、`overlap>=0.35`、最大单次修正 `0.30m/0.20rad` 和 `alpha=0.15` 平滑；`map->odom` 带 `0.10s` 未来容差。连续 ICP 失败阈值为 8 次，DEGRADED 最近可信 TF 宽限为 `5.0s`。活动目标遇到定位短时不可用时，速度门立即输出零速，但适配器等待 `5.0s` 恢复后才取消目标。实测 100 条底盘状态中出现单帧非法 `ctrl_mode=4/252` 和 `ps2_key=255/252`，因此适配器对底盘模式使用 `1.0s` 去抖，持续退出 HOST 才取消目标。Collision Monitor Stop 区为 `0.80x0.64m`，Slow 区为前方缓冲区，Stop/Slow 的 `max_points` 均为 `10`。
