...
- Travel's velocity chain is `/cmd_vel -> localization_cmd_gate -> /cmd_vel_localized -> Collision Monitor -> /cmd_vel_safe -> serial_twistctl`. The gate requires fresh structured localization, Nav2 obstacle cloud, and velocity input; any timeout continuously emits zero. `serial_twistctl` resends zero after 300 ms of command loss, and the STM32 firmware independently clears `Vcx/Wc` with a 500 ms watchdog. The polygon footprint, collision-checked smoothing, and slowdown/stop zones provide close-range safety.
