...
- Travel uses fail-stop Nav2 behavior trees: if `ComputePathToPose`, `ComputePathThroughPoses`, or `FollowPath` fails, the goal stops and fails without automatic spin, backup, or costmap-clearing recovery; close-range avoidance is handled by conservative DWB and a `6 m x 6 m` local costmap.
