...
- a short RTK outage enters bounded `LIO_BRIDGE`: the last trusted `map -> odom` is frozen while FAST-LIO continues `odom -> base_footprint`, so the existing map-frame A* path remains valid at a restricted speed. The guard stops when the 12 s / 5 m bridge budget, local-odom freshness, or LIO jump checks fail; it never promotes LIO to a global-map owner.
