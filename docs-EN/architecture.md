...
- if a trusted pose is only slightly outside that compiled road polygon, the goal manager may perform one bounded `ROAD_REJOIN`: it verifies the nearest graph projection is inside the mask and within `1.0m` of road space, temporarily disables only the **local** KeepoutFilter, and follows that short path at `0.35m/s`. The global filter, RTK authority, stop override, and point-cloud obstacle layer remain active. It restores the local filter before normal route following; an unavailable mask/parameter service, an over-limit drift, a failed re-entry, or a timeout remains fail-closed at zero speed.
...
- **RTK degraded hold**: When RTK fix, heading, or alignment gating genuinely rejects the latest input, `rtk_map_odom_corrector` freezes the last trusted `map->odom` and revokes motion authority; FAST-LIO no longer bridges global localization during RTK degradation. The authority heartbeat can run slightly faster than roughly `10 Hz` FAST-LIO, so a repeated LIO stamp inside `0.30 s` only freezes that correction cycle. GNSS-to-LIO interpolation also uses a `0.30 s` bracket, preventing the observed short scheduling gaps from falsely making a q=4 sample unpairable. During moving reacquisition it retains low-speed `RTK_REACQUIRING` authority instead of stopping spuriously. Truly stale LIO, non-Fixed RTK, and the `2 m / 20 deg` fault boundary remain fail-closed.
