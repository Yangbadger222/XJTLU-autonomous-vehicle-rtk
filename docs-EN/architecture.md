...
- **Corridor-specific Nav2 BT**: Corridor uses a NavigateToPose tree without `Spin` / `BackUp`; planning or tracking failures are stopped and reported by `gps_route_runner` with route-progress context instead of physical recovery actions
...
- **PGO TF freshness**: When no new cloud is available, PGO republishes the latest `map->odom` at timer rate so Nav2 controller/costmap lookups at current time do not fail during low-rate or dropped LIO frames
