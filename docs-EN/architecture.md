...
- In Travel, localizer exclusively owns `map -> odom`. Bundle startup first attempts multi-candidate Scan Context plus ICP; a region service or RViz seed provides fallback. After acceptance, the offset is held while FAST-LIO2 propagates motion; runtime continuous ICP is currently paused.
...
- Travel's velocity chain is `/cmd_vel -> localization_cmd_gate -> /cmd_vel_localized -> Collision Monitor -> /cmd_vel_safe -> serial_twistctl`. The gate requires fresh structured localization, Nav2 obstacle cloud, and velocity input; any timeout continuously emits zero. For nonzero pure-rotation commands with translation no greater than `0.02m/s` and angular speed above `0.01rad/s`, it raises angular speed below the chassis friction range to `0.20rad/s`; zero, numerical noise, and translating commands are unchanged. Travel enables a final acceleration limiter in `serial_twistctl`: zero, deceleration, and safety stops remain immediate, while a nonzero command released after a Stop ramps from zero at `0.30m/s2` and `0.80rad/s2`. `serial_twistctl` resends zero after 300 ms, the STM32 clears commands after 500 ms, and the Foxglove adapter requires `/chassis/status.ctrl_mode=0`.
