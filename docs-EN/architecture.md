...
- Travel uses a fail-stop Nav2 behavior tree: if `FollowPath` or `ComputePathToPose` fails, the goal stops and fails without automatic spin, backup, or costmap-clearing recovery; close-range avoidance is handled by conservative DWB and a `6 m x 6 m` local costmap.
