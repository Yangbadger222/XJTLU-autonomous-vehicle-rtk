...
- RTK gating enters continuous `LIO_BRIDGE`: the last trusted `map -> odom` is frozen while FAST-LIO continues `odom -> base_footprint`, so the existing map-frame A* path remains valid at a restricted speed. Coasting remains available while `/fastlio2/degeneracy` reports healthy geometry; stale, regularized, low-eigenvalue, or jumping LIO latches motion off. A stable RTK return enters `RTK_REACQUIRING`, which moves `map -> odom` back toward RTK online at a lower correction rate.
...
- **RTK degraded hold and LIO bridge**: When RTK fix, heading, or alignment gating rejects the latest input, `rtk_map_odom_corrector` freezes the last trusted `map->odom`. If it had previously reached RTK authority and FAST-LIO remains fresh, `LIO_BRIDGE` continues the same map-frame route at `0.35 m/s` for at most `12 s` and `5 m`; stale local odometry or a `0.50 m` / `15 deg` single-step LIO jump latches motion off until RTK authority recovers.
