...
- **RTK target smoothing and jump gating**: `rtk_map_odom_corrector` adds target deadband + low-pass filtering between the raw RTK target and final per-step limiting, reducing small RTK/heading jitter before it can continuously enter `map->odom`; target-jump gating compares the current raw target against the last trusted raw target, not against the lagging smoothed output
...
- `/localization_authority/*` records the current `map->odom` authority source, rejection reason, raw target jump, raw/output gap, and limited output
