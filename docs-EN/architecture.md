...
- Travel mode bridges RViz `2D Pose Estimate` (`/initialpose`) into `/localizer/relocalize`, and retimes a Nav2-only point cloud as `/fastlio2/body_cloud_nav2` for the local costmap. The global costmap stays static-map based so live point-cloud obstacles cannot mark the robot start cell and block NavFn planning.
