...
- Travel uses the `650x500mm` envelope plus `25mm` polygon margin, 20Hz MPPI with `vx_max=0.30m/s`, a forward slowdown zone, and a close-body stop zone. A stale or non-`LOCALIZED` `/localizer/status` forces zero velocity before serial output
