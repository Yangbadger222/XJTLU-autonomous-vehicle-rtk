...
- A `ROAD_REJOIN_PREPARE` status means the trusted pose is just outside that polygon. The manager accepts recovery only when the closest verified graph edge is on-road and the pose is within `1.0m` of road space. It then limits the command guard to `0.35m/s`, temporarily releases the local filter only, keeps RTK and dynamic-obstacle gates active, and restores the local filter before resuming the A* route. `ROAD_REJOIN_ABORT` or `ROAD_REJOIN_RESTORE_WAIT` is intentionally a stopped state; do not bypass it with an unguarded command.
...
- `nav-gps` uses `/cmd_vel_nav -> guard -> /cmd_vel`. RTK is the only `map->odom` and motion authority; when RTK position/heading gates are not locked, the system freezes the last trusted map transform and publishes zero velocity rather than using FAST-LIO as a global-localization bridge. To reduce stop-go motion in locally obstructed sky view, the Fixed-RTK normal innovation gate is relaxed to `20deg / 1.5m`, the recovery window to `7.5deg / 0.50m`, and short unprocessable input is tolerated for up to `10` failures or `2s`; the `q=4` requirement and `2m / 20deg` fault boundaries remain unchanged.
...
- To evaluate CUDA MPPI against a historical debug bag without starting Nav2 or publishing any command, run `ros2 run mppi_cuda_backend mppi_cuda_bag_replay <bag_path> --output /tmp/mppi_cuda.csv`. The `2026-07-23-16-52-59` `cb_gate` bag contains the required TF, local costmap, A* path, and `/cmd_vel_nav` inputs. This measures the shadow backend only; it does not activate GPU control.
