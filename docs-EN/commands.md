...
- After localizer acceptance, its latched `map -> odom` seed initializes both the authority and AMCL. AMCL candidates must pass covariance, timestamp synchronization, and `0.75m/0.45rad` target-jump gates, then a five-sample window takes the median of at least three consistent candidates. Translation/yaw spread must remain within `0.05m/0.04rad`; stable corrections are released at most once per second with independent `0.05m/0.035rad` deadbands and the existing `0.04m` equivalent base-step cap
