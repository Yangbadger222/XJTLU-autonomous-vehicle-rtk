...
- `nav-gps` uses `/cmd_vel_nav -> guard -> /cmd_vel`. RTK gating after authority has locked enters `LIO_BRIDGE`: it freezes the GPS-derived `map->odom`, retains FAST-LIO `odom->base_footprint`, and caps linear speed at `0.35m/s` while `/fastlio2/degeneracy` remains healthy. The existing map-frame A* route remains active. A stable RTK return switches to `RTK_REACQUIRING`, which corrects the map transform online at the same low speed; stale, regularized, low-eigenvalue, or jumping LIO and authority faults still stop.
