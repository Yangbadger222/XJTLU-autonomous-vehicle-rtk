...
- Travel `NavigateToPose` / `NavigateThroughPoses` use bounded-recovery trees with `0.5Hz` replanning and Savitzky-Golay smoothing. MPPI remains at `20Hz/model_dt=0.05s`, now using closed-loop odometry, fixed sampling noise, and Rotation Shim hysteresis. Collision safety remains enforced by the local costmap, MPPI footprint critic, and Collision Monitor; automatic BackUp remains disabled
