...
- Travel `NavigateToPose` / `NavigateThroughPoses` use dedicated fail-stop behavior trees and follow the NavFn path directly. A controller or planner failure stops and returns failure instead of automatically running `Spin`, `BackUp`, path smoothing, or costmap-clearing recovery actions
- Travel uses the `650x500mm` envelope plus `25mm` polygon margin, 20Hz MPPI with `vx_max=0.45m/s`, a forward slowdown zone, and a close-body stop zone. A stale or non-`LOCALIZED` `/localizer/status` forces zero velocity before serial output
