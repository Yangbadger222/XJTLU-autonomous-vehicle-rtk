...
- Travel `NavigateToPose` / `NavigateThroughPoses` use bounded-recovery trees. Each 1Hz NavFn replan is geometrically Savitzky-Golay smoothed before tracking; collision safety remains enforced by the local costmap, MPPI footprint critic, and Collision Monitor. Planning failure clears the global costmap, controller failure clears the local costmap, and outer recovery performs a bounded wait, collision-checked 90-degree Spin, and clearing sequence. Automatic BackUp remains disabled
