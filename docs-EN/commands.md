...
- The default corridor bag uses the lean profile and records RTK, FAST-LIO2 odom, TF, corridor status, goals, costmaps, `/cmd_vel`, and `/plan`; raw Livox point cloud, Livox IMU, `/fastlio2/body_cloud`, and `/fastlio2/body_cloud_nav2_obstacles` are recorded only with `FYP_CORRIDOR_BAG_PROFILE=debug`
