...
- Status: implementation and shadow-validation phase
- Date: 2026-07-21
...
- [x] Complete the unified driver's 115200 NMEA-only regression on the Jetson, then migrate the sole `/dev/rtk_um982` temporarily to a 921600 mixed stream without persisting receiver state or storing CORS credentials.
...
Implementation note: carrier frequencies follow the UM982 constellation-specific signal-ID table; unknown IDs and invalid GLONASS channels fail closed. GPS/QZSS/Galileo/BDS Kepler states, BDS GEO rotation, GLONASS RK4 propagation, transmit time, and Sagnac rotation are covered by unit tests. Rover and base links now use their own pseudoranges for transmit-time and Sagnac propagation, avoiding centimetre-scale DD model error from sharing the rover satellite state across the approximately 926 m baseline. A zero `OBSVBASE` standard deviation means unavailable and uses conservative 0.30 m code and 0.01 m carrier fallbacks. DD rows sharing one reference satellite are whitened with their full shared-reference covariance instead of being counted as independent scalar observations. DD carrier ambiguities are stored in metres, and their keys contain the target/reference satellites plus all four rover/base arc IDs, so a reference change or one-satellite slip cannot silently reuse the old variable. The Eigen smoother relinearizes Phase 3 ECEF IMU propagation and Phase 4 raw line/plane factors together with DD code/carrier factors. It bounds the window by time and state count, then Schur-marginalizes old states and orphan ambiguities into an anchored dense prior; it never clears and rebuilds the graph with an artificial strong prior.

The Phase 5 ROS path uses `fgo_gil_msgs/LidarConstraintBatch` so the Phase 4 frontend passes actual line/plane factors instead of a FAST-LIO pose factor. `system_fgo_gil_float.launch.py` retains `/fgo_gil/float_odom_ecef`; Phase 6 only adds the separate `/fgo_gil/fixed_odom_ecef`. It never publishes TF or commands. `calibration.ecef_from_lidar_world.calibrated` and the static `calibration.gnss.base_ecef_calibrated` override default to `false`. The base gate can now be satisfied by a CRC-valid RTCM 1005/1006 message; ECEF/world still requires a validated field fit. The master lever-arm placeholder `[0.0,-0.184,0.134] m` is derived from the measured right antenna `[0.0,-0.184,0.154] m` in `base_link` minus the current uncalibrated IMU Z placeholder `0.02 m`.
...
Integer resolution first locks to the newest GNSS state in the graph and requires one unique reference-satellite and rover/base reference-arc basis per signal group at that state. Current variables from different constellations/signals are each converted to cycles, then enter LAMBDA jointly with their full cross-covariance. On a reference change, a new variable receives the transformed mean initialization `N_i^q=N_i^r-N_q^r`, but it does not inherit the old information matrix and must accumulate observations again; any related arc change also creates a new variable. By default an ambiguity needs five continuous valid epochs before it can enter LAMBDA. This deliberately avoids overconfidence from an unmodelled covariance basis transform, at the cost of a short FLOAT-only period after a reference change. Base observations allow a 5 s gap while rover observations retain 2 s, so a continuous lock is not discarded by normal jitter in the 1 Hz CORS stream. GLONASS integer fixing is explicitly excluded in this phase because FDMA target/reference wavelengths differ and the metre-domain combination is not one integer cycle variable.

Default gates are `ratio>=3.0`, bootstrap success rate `>=0.99`, and normalized candidate squared norm `<=25`. If the full set fails, the largest-variance ambiguity is removed iteratively for partial fixing, down to four ambiguities. An accepted integer candidate only produces the conditional preview `delta_x=P_xa P_aa^-1(a_fixed-a_float)`; excessive position, attitude, velocity, or graph-cost change rejects it. A candidate that passes the numerical gates and back-substitution must also retain the same ambiguity-key set and integer-cycle values for three consecutive new GNSS epochs. Until then it reports `CONFIRMATION_PENDING` and publishes no fixed output. Any rejection, candidate change, or graph reset clears the confirmation count. Back-substitution never writes into the float graph, whether accepted or rejected. The float topic always carries the original solution. Each newly added batch of DD factors triggers one fixed candidate after the next successful graph optimization; the GNSS factors may be attached to the closest LiDAR state that remains in the lag window. Without new GNSS factors, old ambiguities cannot republish a stale fix.
...
Further clean replay of the same bag on 2026-07-16 exposed and fixed two independent software defects. First, the LiDAR rotation Jacobian used an absolute ECEF point at roughly 6378 km even though the smoother applies independent position and attitude perturbations, driving the condition estimate to `1.86e18`. It now uses only the rotated body-frame point and is protected by an Earth-radius regression. Second, the single-thread executor blocked IMU/raw callbacks while optimizing. IMU, raw input, and estimator work now use separate callback groups with a three-thread executor, bounded pending queues, and fail-closed reset/reseed when IMU history is unavailable or optimization fails. Raw epochs are drained before a keyframe instead of triggering one redundant optimization per message. Deterministic uniform sampling retains at most 48 line and 96 plane factors per keyframe. Repeated finite differences propagate only the nominal IMU state, and the package defaults to `RelWithDebInfo` when no build type is specified. A non-finite graph condition estimate, or one above `1e12`, rejects the update and resets instead of publishing a bad state.

These queue and gate values are replay-constrained safety defaults: `imu_qos_depth=512` retains about 2.56 s at roughly 200 Hz; raw DDS depth is 512 and the in-node raw/ephemeris queues are 1024/64; LiDAR may wait for at most 16 batches and 0.5 s. The factor caps remove repeated constraints of the same class without weakening the frontend geometry gates. Do not shrink them merely to reduce latency; future tuning requires accuracy and degeneration comparisons on a new GNSS-complete bag.

The final 287 s input-only replay used `scripts/replay_fgo_gil_bag.sh`, explicitly excluding old `/fgo_gil/*` outputs stored in the source bag so stale constraints and diagnostics cannot contaminate the current nodes. It produced 496 LiDAR keyframe batches, 496 float ECEF outputs, and 496 unified odometry/path outputs. The final graph contained 10 states and 1929 factors over 9.40 s after 486 marginalizations, with a condition estimate near `7.85e4`. Optimization failures, rollbacks, numerical rejections, IMU reseeds, FGO pending/drops, and raw queue drops were all zero. Across 326 performance samples in the playback window, optimization latency mean/p95/max was `120.2/141.2/151.2 ms`; finite output-age p95/max was `0.744/1.046 s`. The output bag contains 4364 messages and passes SQLite `integrity_check=ok`.

This is still a LiDAR/IMU software-continuity regression, not paper-level positioning acceptance. The input has only four Galileo ephemerides and `calibration.gnss.base_ecef_calibrated=false`, so none of the 287 aligned GNSS epochs entered GNSS fusion; the steady-state status was `LIO_ONLY_WAITING_BASE`. No GNSS code/carrier factor was created, and meaningful ECEF APE/RPE or fixing rate cannot be computed. FGO remains shadow-only, publishes no production TF, and does not affect FAST-LIO2/Nav2.

The 2026-07-21 outdoor moving bag is the first five-system raw dataset on this branch. Its SHA-256-verified 338.17 s copy contains 98,996 messages: 67,604 IMU samples at 199.91 Hz, 3,382 raw LiDAR frames, 3,364 FAST-LIO poses, 458 LiDAR constraint batches, 3,378 master epochs, 3,378 secondary epochs, 338 base epochs, and 284 ephemerides. GPS/GLONASS/Galileo/BDS/QZSS ephemerides cover 11/8/10/14/3 satellites. All 338 base epochs exactly match a master epoch by GNSS week/TOW. The old evaluator matched only 153 because it incorrectly used variable reception timestamps; evaluation now pairs by week/TOW and retains reception time only for ROS-trajectory association.

One isolated system event at bag offset 47.78 s produced a 313.7 ms IMU gap, three missing master epochs, four missing secondary epochs, a roughly 405.6 ms LIO interval, and one UM982 mixed-stream CRC failure reported at 48.35 s. All streams recovered, no overflow or decode failure followed, and one-second `tegrastats` around the event showed no memory, swap, thermal, or sustained CPU limit. The bag is useful for calibration geometry and continuity debugging but fails the strict continuous-IMU acceptance gate.

The unified driver now observes the same NTRIP byte stream that it forwards unchanged to UM982 through a bounded RTCM3 framer. CRC24Q-valid type 1005/1006 messages publish `/gnss/rtcm/reference_station`. A static calibrated base parameter has priority; otherwise the first dynamic coordinate opens the base gate. A station ID, ITRF realization, NTRIP source, or ECEF change above 1 cm resets the epoch aligner, ambiguity arcs, pending GNSS data, and shadow graph. The `calibrate_fgo_gil_ecef_world.py` tool fits the remaining no-scale SE(3) transform from GGA quality-4 `/fix` and FAST-LIO poses after applying the existing IMU-LiDAR and antenna lever arms. It rejects missing quality evidence, short/straight motion, insufficient pairs, and excessive residuals.
...
The 2026-07-21 carrier/performance replay confines each DD-group correlated-covariance whitening operation to its local variable block. Code factors allocate a `rows x 6` Jacobian; carrier factors allocate `rows x (6 + ambiguity_count)`, then scatter the local gradient/Hessian through global scalar offsets. Shared-reference covariance, group Mahalanobis-RMS Huber weighting, and the resulting graph model are unchanged. On the same 418.5 s direct-constraint 1x bag, optimization mean/p95/max latency fell from `163.83/305.17/450.25 ms` to `120.28/160.28/181.68 ms`, while RTF p95 fell from `0.564` to `0.310`. The 567 outputs, `96.98%` availability, and `0.2355/0.1025 m` APE/RPE RMSE remained consistent.

The same replay adds newest-factor carrier raw/normalized residuals, effective sigma, arc observation counts, fix-eligible/candidate dimensions, and confirmation count per `(constellation, signal_type, l2c)`. Effective sigma is mostly `14--22 mm`, but median residual RMS is `0.555 m` (`38.9 sigma`) for GPS signal 0 and `0.760 m` (`50.5 sigma`) for Galileo signal 12; several signals have multi-metre p95 residuals. Confirmation reached at most one epoch and no fixed output was published. This identifies carrier modelling, inter-receiver timing/atmospheric terms, or arc consistency as the active blocker rather than overly strict LAMBDA gates. Vehicle work must remain diagnostic shadow mode and cannot own `map->odom`.

The apparent metre-level carrier fault above was traced to a UM982 convention mismatch: its binary field is accumulated Doppler range (ADR), whose sign is opposite the positive-range carrier phase used by the DD model. The FGO input adapter now negates ADR while the raw driver preserves the receiver-native value. On the same 418 s bag, median post-fit carrier RMS fell to `2.9--7.9 mm` across the principal groups and normalized medians fell below `0.54 sigma`; APE/RPE stayed at `0.235/0.102 m`. Raw code/carrier DD changes also have strong negative correlation before normalization, independently confirming the convention.

Integer fixing still fails closed. DD code residual medians range from `0.44 m` to `11.06 m`, the GGA-to-LIO ECEF fit has `0.225 m` RMSE, and the final four-dimensional partial set repeatedly contains two GPS signal 14, one Galileo signal 12, and one Galileo signal 17 ambiguity. Its fractional distance is `0.096--0.162 cycle` (roughly `2.5--4 cm`) even when the ratio is `4.19--13.06`. The previous covariance contained receiver-reported phase noise and the unavailable-base fallback but no baseline/elevation-dependent residual atmosphere or multipath term. `gnss.carrier_model_sigma_zenith_m_per_km` now adds a configurable target/reference single-difference variance scaled by `baseline_km/sin(elevation)`; the shared-reference covariance remains intact. This models the missing uncertainty instead of weakening the LAMBDA residual threshold.

At the `0.01 m/km` default, median effective carrier sigma increased from roughly `14--16 mm` to `20--28 mm`; APE/RPE remained `0.235/0.102 m`, latency p95 remained about `161 ms`, and the isolated ratio failure disappeared. The same eight partial sets still failed the squared-norm residual test. A parameter-only `0.02 m/km` comparison produced the same eight failures and no fixed output, so the default is not increased further. The remaining stable L5/E5 fractional bias must be diagnosed by exact PRN/arc and antenna/receiver/VRS phase-bias evidence; inflating covariance further would only conceal it.

Exact failed-set diagnostics then showed identical arcs and satellite pairs over consecutive evaluations: GPS signal 14 used reference PRN 24 with targets 18 and 23, while Galileo signals 12/17 both used reference PRN 34 and target PRN 3. The GPS18 fractional value drifted from `+0.122` to `+0.214 cycle`; Galileo E5a drifted from `-0.102` to `-0.131`, and E5b from `-0.053` to `-0.099`. This is not a constant quarter-cycle offset. The float ambiguity is continuing to absorb changing state/map geometry.

A controlled replay raised line/plane sigma from `0.05 m` to `5 m` without changing code or integer gates. Ratio/residual failures disappeared and three numerical fixed solutions were accepted, but APE/RPE against FAST-LIO degraded from `0.235/0.102 m` to `0.389/0.273 m`; no sustained FIXED status was published. This proves the fixed FAST-LIO-world anchors transformed by one offline `ecef_from_lidar_world` are fighting GNSS global correction. Permanently weakening LiDAR is rejected. The next architectural repair must make local-map-to-ECEF alignment an estimated, fixed-lag-consistent quantity (or rebuild map anchors from FGO states), so LiDAR preserves local shape while GNSS can move the global frame.

The repair adds one shared six-DoF `ECEF <- FAST-LIO world` variable to the fixed-lag graph. LiDAR residuals and their analytic state/alignment Jacobians are evaluated in FAST-LIO world, the alignment variable is retained in the Schur marginal prior, and ordinary optimizer/IMU reseeds inherit the last valid alignment. Only a real FAST-LIO frontend epoch change returns to the offline calibration. Fixed-ambiguity preview also applies and rolls back the correlated alignment correction, so candidate cost checks remain consistent without mutating the float graph.

The same 1x bag exposed why the alignment prior cannot be arbitrary. A weak `10 m / 0.1 rad` prior let noisy DD code move the map by `2.535 m / 0.020 rad`, producing `0.673/0.115 m` APE/RPE and no fixed output. The offline fit used 3964 inliers with `0.225 m` point RMSE and `0.461 m` maximum residual, so the aggregate transform is much better known than an individual fix. With the validated `0.05 m / 0.005 rad` prior, replay retained 567 outputs and 96.98% availability, produced `0.299/0.106 m` APE/RPE, `167 ms` optimization-latency p95, and no optimization failure. Consecutive attempts 46--48 used the same six-dimensional integer set: two were `CONFIRMATION_PENDING` and the third produced one confirmed FIXED output. Five pending, one confirmed, and one later residual rejection are useful architecture evidence, but not sustained-FIXED or vehicle acceptance.

`evaluate_fgo_gil_bag.py` time-matches FGO and FAST-LIO comparator poses, then applies a no-scale SE(3) rigid alignment so ECEF and local coordinates are not directly subtracted. Outage analysis uses one-to-one master/base epochs paired by GNSS week/TOW within 50 ms; reception timestamps are diagnostic only. A one-sided raw stream reports `RAW_GNSS_INCOMPLETE`. It reports APE/RPE, availability, fixing rate, outage drift, optimization latency/RTF, and `tegrastats` CPU/RAM. Metadata-only mode needs no ROS deserialization; a missing raw topic or zero raw messages explicitly produces `RAW_GNSS_UNAVAILABLE`.

### Phase 8: Weather-dependent collection and acceptance

- [x] Implement bounded RTCM3 framing, CRC24Q validation, and type 1005/1006 reference-station publication; retain the NTRIP byte stream unchanged for UM982.
...
- [ ] Confirm the live CORS station coordinate, station ID, baseline distance, and RTCM content from `/gnss/rtcm/reference_station` in a new bag.
