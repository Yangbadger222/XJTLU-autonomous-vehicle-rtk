...
The 2026-07-23 `cb_gate` bag measured FAST-LIO header gaps of `0.157s` P95 and `0.253s` maximum while the corrector heartbeat ran at `10Hz`. `max_lio_age_s` is therefore `0.30s`, covering observed Jetson scheduling jitter; a fresh repeated stamp only holds the last TF. During moving reacquisition, that repeated sample preserves reduced-speed `RTK_REACQUIRING` authority while waiting for the next distinct LIO frame instead of incorrectly stopping q=4 RTK navigation. A gap beyond that window, non-Fixed RTK, or a true fault remains fail-closed.
