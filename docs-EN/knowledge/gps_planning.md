...
`gps_route_runner` now consumes stamped LIO separately from `map->odom`. Non-finite/regressing local odom or rates above `10 m/s`/`10 rad/s` abort immediately; rates above `3 m/s`/`3 rad/s` abort after three consecutive samples. The global-correction rate no longer compares raw `map->odom`: far from the odom origin, a small yaw release can appear as several metres per second of raw TF translation through the lever arm. The runner now applies old and new `map->odom` to the same current `odom->base`, then evaluates the actual base-space correction step against `0.50 m/s` / `5 deg/s` for `GLOBAL_CORRECTION_HOLD`. False/stale motion authority still enters the same hold and is never reported as `ODOM_DIVERGENCE_ABORT`.
...
The 2026-07-23 `cb_gate` bag measured FAST-LIO header gaps of `0.157s` P95 and `0.253s` maximum while the corrector heartbeat ran at `10Hz`. Both `max_lio_age_s` and the GNSS-to-LIO `max_odom_bracket_s` are therefore `0.30s`, covering observed Jetson scheduling jitter; a fresh repeated stamp only holds the last TF. During moving reacquisition, that repeated sample preserves reduced-speed `RTK_REACQUIRING` authority while waiting for the next distinct LIO frame instead of incorrectly stopping q=4 RTK navigation. A gap beyond that window, non-Fixed RTK, or a true fault remains fail-closed.
