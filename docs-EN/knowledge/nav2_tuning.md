...
11. The `20-37-08` Travel bag proved raw NavFn geometry was the primary weaving source: the first `4.23m` path accumulated `15.23rad` of heading changes with 30 significant corners, while the vehicle traveled `5.73m` for only `3.86m` displacement and reversed angular-command direction 46–52 times. Travel now applies Savitzky-Golay `SmoothPath` after every replan. The `20-58-24` bag then proved the smoother's global-footprint collision recheck rejected every path, so that duplicate check is disabled while the local costmap, MPPI footprint critic, and Collision Monitor continue enforcing motion safety. The local costmap is reduced from `15Hz` to `10Hz`. The localization gate raises only nonzero pure rotations below the chassis range to `0.20rad/s`, and FAST-LIO2 adds a `0.05s` TF future tolerance.
