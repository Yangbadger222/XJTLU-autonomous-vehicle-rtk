...
- The nav-gps local costmap keeps obstacle updates at `8Hz` while publishing at `2Hz`; the global costmap uses `2Hz/1Hz`. At the `2.0m/s` speed limit, 8Hz refreshes obstacles every 0.25m while reducing costmap serialization and DDS work.
