...
9. Travel is a separate indoor prior-map profile. Its global costmap uses only static-map and inflation layers; the local costmap reads the retimed `/fastlio2/body_cloud_nav2` obstacle cloud. It uses the low-load MPPI limit (`0.35m/s`, `0.65rad/s`, `15Hz`, `batch_size=500`) and a `6m x 6m @ 0.05m` rolling local map.
