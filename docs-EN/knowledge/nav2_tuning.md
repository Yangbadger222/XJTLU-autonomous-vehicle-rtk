...
9. Travel is a separate indoor map-bundle profile. The global costmap uses static map/inflation only and the local costmap reads `/fastlio2/body_cloud_nav2`. Its acceptance profile uses `vx_max=0.30m/s`, `vx_std=0.16`, `ax_max=0.40m/s2`, `20Hz`, `batch_size=300`, and `regenerate_noises=true`. During the 2026-07-12 vehicle run, `batch_size=500` continuously missed the 20Hz control period, so it was reduced to 300 and critic-stat publication was disabled. `model_dt=0.05s` remains unchanged, so lowering controller frequency is not a valid workaround. Rotation Shim aligns path-heading errors above `0.35rad` at `0.30rad/s`, while `VelocityDeadbandCritic` avoids the unreliable breakaway range. The measured `650x500mm` body plus `25mm` per side remains the planning footprint. The obstacle cloud uses a `0.80x0.60m` self-filter for measured body returns, while Collision Monitor keeps its `0.80x0.64m` stop polygon. A wider `0.90x0.70m` stop polygon repeatedly classified traversable side returns at `y=-0.33~-0.35m` as stops during the vehicle test; the local costmap's measured footprint continues to check lateral swept clearance.
10. Travel keeps fail-stop trees without automatic spin, backup, or full-map clearing. Localization runs continuous ICP at 1Hz with `overlap>=0.35`, a `0.30m/0.20rad` correction jump limit, `alpha=0.15` smoothing, and `0.10s` future tolerance on `map->odom`. One ICP failure may use the latest trusted TF for up to `2.5s` in DEGRADED state. If localization becomes temporarily unavailable during an active goal, the command gate immediately publishes zero while the adapter waits up to `2.5s` for recovery before canceling; sustained LOST remains fail-stop. Collision Monitor uses a `0.80x0.64m` stop zone and a forward `x=0.45~0.85m, y=+-0.30m` slowdown buffer at 60%. Static wide-corridor measurements found `0-6` sparse stop-zone returns per frame with a `2.39` average, so Stop and Slow both use `max_points=10`; a real planar obstacle still produces a dense triggering cluster.
