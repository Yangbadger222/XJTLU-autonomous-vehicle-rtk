...
- `build_scene_runtime.py` splits proper internal route-edge crossings into explicit junction nodes and four edge segments. A* can therefore turn where QGIS lines visually cross but the original bundle did not share an endpoint; shared endpoints, near-parallel roads, and grade-separated roads are not auto-connected.
