...
10. Travel uses bounded-recovery trees: 1Hz replanning, global clearing after planner failure, local clearing after controller failure, and up to five outer retries rotating through a one-second Wait, collision-checked 90-degree Spin, and local/global clearing. BackUp and path smoothing remain disabled. Localization runs continuous ICP at 1Hz with `overlap>=0.35`, a `0.30m/0.20rad` correction jump limit, `alpha=0.15` smoothing, and `0.10s` future tolerance on `map->odom`. It tolerates eight consecutive ICP failures and retains the latest trusted TF for up to `5.0s` in DEGRADED state. The command gate immediately publishes zero during localization loss while the adapter waits up to `5.0s` before cancellation. A 100-message chassis sample contained isolated invalid `ctrl_mode=4/252` and `ps2_key=255/252` frames, so chassis mode is debounced for `1.0s`. Collision Monitor keeps its stop and forward slowdown zones with `max_points=10`.
