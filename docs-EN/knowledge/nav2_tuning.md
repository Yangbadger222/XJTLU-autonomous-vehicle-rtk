...
9. Travel is a separate indoor map-bundle profile. The global costmap uses static map/inflation only and the local costmap reads `/fastlio2/body_cloud_nav2`. Low-load MPPI remains `0.35m/s`, `0.65rad/s`, `20Hz`, `batch_size=500`; the `650x500mm` envelope plus `25mm` per side is configured as a polygon, `CostCritic.consider_footprint=true`, terminal yaw tolerance is `0.20rad`, and SmoothPath collision checking is enabled. The envelope and tolerances still require narrow-door and in-place-turn vehicle acceptance.
10. Travel keeps fail-stop trees without automatic spin, backup, or full-map clearing. Localization runs continuous ICP at 1Hz with `overlap>=0.35`, a `0.30m/0.20rad` correction jump limit, and `alpha=0.15` smoothing. Non-`LOCALIZED` state is stopped by the velocity gate, followed by Collision Monitor `0.65x0.48m` slowdown and `0.40x0.32m` stop zones. These provisional zones must be tuned to Livox blind range and measured braking distance.
