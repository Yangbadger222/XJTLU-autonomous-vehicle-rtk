...
- The nav-gps local costmap keeps obstacle updates at `8Hz` while publishing at `2Hz`; the global costmap uses `2Hz/1Hz`. At the `0.85m/s` speed limit, 8Hz still refreshes obstacles about every 0.11m while reducing costmap serialization and DDS work.
