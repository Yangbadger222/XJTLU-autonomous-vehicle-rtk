...
9. Travel is a separate indoor map-bundle profile. It keeps `20Hz/model_dt=0.05s` and `time_steps=40`; controller frequency cannot be lowered independently of the model step. The `21-45-44` bag recorded 146 missed 20Hz periods, so `batch_size` is reduced from 200 to 160 while retaining the two-second horizon. Because the `21-06-57` bag still showed frequent steering reversals, it uses `regenerate_noises=false`, `wz_std=0.10`, and Rotation Shim hysteresis with `angular_dist_threshold=0.35`, `angular_disengage_threshold=0.15`, and `closed_loop=true`. Humble MPPI does not declare `open_loop`, so the configuration does not retain an ineffective key. The measured footprint and obstacle geometry remain unchanged.
10. Travel uses bounded recovery with `0.5Hz` replanning, global/local clearing, bounded Wait and collision-checked Spin, and no BackUp. PCD ICP validates startup only; the accepted `map->odom` offset is then held while FAST-LIO2 propagates motion. Chassis mode keeps its `1.0s` debounce and Collision Monitor Stop/Slow retain `max_points=10`.
11. The `20-37-08` bag proved raw NavFn geometry was jagged, so Travel enabled Savitzky-Golay `SmoothPath`; `20-58-24` proved the smoother footprint recheck rejected every path, so that duplicate check was disabled. In `21-06-57`, the first run succeeded and trajectory efficiency improved from `0.67` to `0.82`; the second met a real obstacle and fell into the chassis deadband under Slow. The `21-45-44` bag then showed Collision Monitor changed only 2/2260 commands. The actual stalls were 30-second and 10-second pure-rotation phases where Nav2 continuously emitted about `0.04rad/s`, LIO showed almost no displacement or rotation, and the translation-only progress checker failed three times. Every nonzero pure-rotation command is therefore raised unconditionally to at least `0.20rad/s`; a mixed Slow turn is still converted to in-place rotation only after confirmed slowdown. `PoseProgressChecker` now counts `0.15rad` of rotation as progress. Global inflation uses the minimum safe `0.30m`, still above the `0.285m` inscribed radius, and FAST-LIO2 retains its `0.05s` TF future tolerance.
