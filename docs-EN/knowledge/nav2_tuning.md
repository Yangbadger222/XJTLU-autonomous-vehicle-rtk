...
9. Travel is a separate indoor map-bundle profile. The global costmap uses static map/inflation only and the local costmap reads `/fastlio2/body_cloud_nav2`. The first vehicle run showed that `0.35m/s`, `vx_std=0.14`, and the 35% slowdown combined to send only about `0.02-0.034m/s` over serial, below chassis breakaway. The low-load MPPI profile is therefore adjusted to `0.45m/s`, `vx_std=0.20`, `ax_max=0.70m/s2`, `20Hz`, `batch_size=500`, with `regenerate_noises` enabled. The `650x500mm` envelope plus `25mm` per side remains the polygon footprint, `CostCritic.consider_footprint=true`, and terminal yaw tolerance remains `0.20rad`.
