...
10. Travel keeps fail-stop trees without automatic spin, backup, or full-map clearing. Localization runs continuous ICP at 1Hz with `overlap>=0.35`, a `0.30m/0.20rad` correction jump limit, `alpha=0.15` smoothing, and `0.10s` future tolerance on `map->odom`. It tolerates eight consecutive ICP failures and retains the latest trusted TF for up to `5.0s` in DEGRADED state. If localization becomes temporarily unavailable during an active goal, the command gate immediately publishes zero while the adapter waits up to `5.0s` for recovery before canceling; sustained LOST remains fail-stop. A 100-message chassis sample contained isolated invalid `ctrl_mode=4/252` and `ps2_key=255/252` frames, so the adapter debounces chassis mode for `1.0s` and cancels only when HOST mode remains absent. Collision Monitor uses a `0.80x0.64m` stop zone and a forward slowdown buffer. Static wide-corridor measurements found `0-6` sparse stop-zone returns per frame with a `2.39` average, so Stop and Slow both use `max_points=10`; a real planar obstacle still produces a dense triggering cluster.
