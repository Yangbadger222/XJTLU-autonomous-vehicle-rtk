...
11. The `20-37-08` bag proved raw NavFn geometry was jagged, so Travel enabled Savitzky-Golay `SmoothPath`; `20-58-24` proved the smoother footprint recheck rejected every path, so that duplicate check was disabled. In `21-06-57`, the first run succeeded, trajectory efficiency improved from `0.67` to `0.82`, and safe steering reversals dropped from 46 to 24. The second run met a real obstacle, remained in Slow, and fell into the chassis deadband at `vx=0.04–0.07m/s,wz≈0.20rad/s`. Deadband handling now runs after Collision Monitor: a low-speed turning request becomes zero translation plus at least `0.20rad/s` in-place rotation, while Stop and zero commands remain unchanged. Global inflation is corrected from `0.25m`, below the footprint inscribed radius, to `0.35m` so plans do not start by hugging obstacles. FAST-LIO2 retains its `0.05s` TF future tolerance.
