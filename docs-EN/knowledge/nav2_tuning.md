...
9. Travel remains a separate indoor map-bundle profile. MPPI must stay at `20Hz/model_dt=0.05s`; it cannot be lowered independently to 15Hz. The latest `22-03-41` bag missed 58 periods during about 110.5 seconds of active control, and Humble CostCritic scores all `batch_size*time_steps` points serially. `time_steps` therefore falls from 40 to 32 while `batch_size` stays 160, reducing candidate points per cycle from 6400 to 5120. PreferForwardCritic is redundant with `vx_min=0`, and Humble does not declare CostCritic `trajectory_point_step`, so both are removed. The local costmap still updates internally at 10Hz but publishes at 2Hz; the static global costmap updates/publishes at 1Hz to reduce serialization and bag load.
10. Rotation Shim now handles only large initial errors above `0.65rad`, hands control back at `0.35rad`, and does not own final heading. VelocityDeadbandCritic drops from weight 35 and `[0.14,0,0.18]` to 15 and `[0.12,0,0.08]`; PathAlign drops from 12 to 6 and PathFollow from 16 to 12. The conditioner no longer raises every pure rotation to `0.20rad/s`; only a Collision Monitor-confirmed Slow command whose mixed linear component falls below breakaway is converted to translation-free rotation while preserving angular velocity. This removes the excessive intervention seen in `22-03-41`, where post-processing changed 55.9% of commands and made 4433 of them pure rotations.
11. Travel replans at 1Hz. A FollowPath failure first clears the local costmap and immediately replans/smooths; the outer recovery repeats clear/replan before a short `0.52rad` Spin is allowed only under `IsStuck`, followed by Wait/final clearing and never BackUp. `prior_map_tf_authority` is the sole `map->odom` owner: the localizer supplies the startup seed, AMCL supplies runtime candidates, and covariance, timestamp, target-jump, TF-step, and equivalent `map->base` step gates release corrections smoothly. This addresses the roughly 1m late-corridor drift seen previously without permitting abrupt TF jumps.
