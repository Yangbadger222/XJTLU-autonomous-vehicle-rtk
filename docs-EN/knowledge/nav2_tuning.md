...
The RTK driver now treats `THS`, `HPR`, and `UNIHEADING` as competing heading sources instead of publishing whichever serial sentence arrives last. Production configuration prefers `THS`; `UNIHEADING` becomes eligible only after `THS` has been absent for `1.5 s`, and three consecutive fallback samples are required before the handoff. At the handoff, the fallback is aligned to the last trusted heading and every continuing stream is limited to `75 deg/s`, `15 deg` per sample, plus a `2 deg` slack. The driver reports the selected source, applied continuity bias, and rejected-heading count in `/rtk/status`.

This is required because the 2026-07-24 nav-gps bag stayed RTK Fixed while its heading changed from `THS 346.2 deg` to `UNIHEADING 70.3 deg`. A positional q=4 solution does not make a mismatched heading convention safe for `map -> odom`.
