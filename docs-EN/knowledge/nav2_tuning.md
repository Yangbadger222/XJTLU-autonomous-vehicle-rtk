...
9. Travel is a separate indoor prior-map profile. Its global costmap uses only static-map and inflation layers; the local costmap reads the retimed `/fastlio2/body_cloud_nav2` obstacle cloud. It uses the low-load MPPI limit (`0.35m/s`, `0.65rad/s`, `20Hz`, `batch_size=500`) and a `6m x 6m @ 0.05m` rolling local map. The `20Hz` loop matches `model_dt=0.05s`, preventing MPPI from rejecting a controller period larger than its model step.
10. Travel uses fail-stop NavigateToPose and NavigateThroughPoses trees. Automatic spin, backup, and costmap-clearing recovery actions are unavailable. The ICP localizer owns `map->odom`; after successful relocalization its correction is frozen and republished with the current ROS time. Verify cloud-to-map alignment in RViz before each goal.
