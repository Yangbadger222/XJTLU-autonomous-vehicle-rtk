...
9. Travel is a separate indoor map-bundle profile. The global costmap uses static map/inflation only and the local costmap reads `/fastlio2/body_cloud_nav2`. The first vehicle run showed the old 35% slowdown reducing serial commands to about `0.02-0.034m/s`; after correcting the slowdown zone, MPPI instead remained pinned at `0.45m/s`. The indoor acceptance profile is therefore set to `vx_max=0.30m/s`, `vx_std=0.16`, `ax_max=0.40m/s2`, `20Hz`, `batch_size=500`, and `regenerate_noises=true`. Travel wraps MPPI in Rotation Shim: a path-heading error above `0.35rad` triggers an in-place alignment at `0.30rad/s` before handing control to MPPI. `VelocityDeadbandCritic` uses `[0.14, 0.0, 0.18]` to penalize trajectories below the vehicle's reliable breakaway range, avoiding tiny simultaneous linear/angular commands that hold one wheel in the PID-controlled static-friction region. A final serial limiter caps acceleration recovery at `0.30m/s2` and `0.80rad/s2` without delaying zero or deceleration. The `650x500mm` envelope plus `25mm` per side remains the polygon footprint, `CostCritic.consider_footprint=true`, and terminal yaw tolerance remains `0.20rad`.
10. Travel keeps fail-stop trees without automatic spin, backup, or full-map clearing. Vehicle logs showed Savitzky-Golay moving otherwise plannable NavFn paths into collision, so Travel now follows the collision-feasible NavFn path directly. Localization runs continuous ICP at 1Hz with `overlap>=0.35`, a `0.30m/0.20rad` correction jump limit, and `alpha=0.15` smoothing. Non-`LOCALIZED` state is stopped by the velocity gate. Collision Monitor retains the `0.40x0.32m` stop zone; its slowdown zone is a forward `x=0.45~0.85m, y=+-0.30m` buffer with a 60% ratio, preventing rear/side cloud returns from permanently throttling the forward-only Travel controller.
