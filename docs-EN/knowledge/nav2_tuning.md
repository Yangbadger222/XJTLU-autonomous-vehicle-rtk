...
11. Travel replans at 1Hz. A FollowPath failure first clears the local costmap and immediately replans/smooths; the outer recovery repeats clear/replan before a short `0.52rad` Spin is allowed only under `IsStuck`, followed by Wait/final clearing and never BackUp. `prior_map_tf_authority` remains the sole `map->odom` owner. In addition to covariance, synchronization, and jump gates, AMCL must pass a five-sample/three-consistent-sample window; its stable median is released at most once per second with independent translation/yaw deadbands. Offline replay of the two latest bags reduces TF changes from `31/35` to `7/6` and cumulative yaw from about `5.1deg` to `0.31/1.20deg`, while still releasing about `5~6cm` of stable translation correction.
