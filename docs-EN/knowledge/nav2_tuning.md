...
9. Travel is a separate indoor map-bundle profile. The global costmap uses static map/inflation only and the local costmap reads `/fastlio2/body_cloud_nav2`. Its acceptance profile uses `vx_max=0.30m/s`, `vx_std=0.16`, `ax_max=0.40m/s2`, `20Hz`, `batch_size=200`, `time_steps=40`, and `regenerate_noises=true`. During the 2026-07-12 vehicle run, both the original 500 batch and the reduced 300 batch missed the 20Hz control period; the 300 profile also reported `Optimizer fail to compute path` after about five seconds. Sampling is therefore reduced to `200x40`, with transient controller failure tolerance raised from `0.6s` to `2.0s`. `model_dt=0.05s` remains unchanged. Rotation Shim and the deadband critic remain enabled. The measured planning footprint remains unchanged, while the obstacle cloud combines its `0.80x0.60m` self-filter with the measured narrow 3D body-reflection patch; the local costmap footprint continues to check swept clearance.
10. Travel uses bounded-recovery trees: 1Hz replanning, global clearing after planner failure, local clearing after controller failure, and up to five outer retries rotating through a one-second Wait, collision-checked 90-degree Spin, and local/global clearing. BackUp remains disabled. During the 2026-07-12 vehicle run, continuous ICP withdrew `map->odom` after about 17 seconds of motion, causing repeated TF extrapolation and goal cancellation. Travel therefore performs PCD ICP only for startup pose validation, then holds the accepted `map->odom` offset while FAST-LIO2 propagates motion through high-rate `odom->base_footprint`. The transform retains `0.10s` future tolerance; runtime map matching must be repaired and revalidated before long-duration LIO drift acceptance. Chassis mode remains debounced for `1.0s`, and Collision Monitor Stop/Slow use `max_points=10`.
11. The `20-37-08` Travel bag proved raw NavFn geometry was the primary weaving source: the first `4.23m` path accumulated `15.23rad` of heading changes with 30 significant corners, while the vehicle traveled `5.73m` for only `3.86m` displacement and reversed angular-command direction 46–52 times. Travel now applies collision-checked Savitzky-Golay `SmoothPath` after every replan and reduces the local costmap from `15Hz` to the obstacle cloud's `10Hz`. The same bag showed repeated `0.04rad/s` commands after entering the `0.35m` goal radius, below the chassis' effective rotation range. The localization gate now raises only nonzero pure-rotation commands to `0.20rad/s`; zero commands, translating motion, and safety stops are unchanged. FAST-LIO2 adds a `0.05s` future tolerance to `odom->base_footprint` TF to cover scheduling skew between 10Hz LIO and the 20Hz controller.
