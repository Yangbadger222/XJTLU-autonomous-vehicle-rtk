...
- In the merged `frc` TF layout, FAST-LIO2 publishes `odom -> base_footprint` and the URDF publishes `base_footprint -> base_link`; therefore `rtk_fgo.yaml` sets `frames.base_link` to `base_footprint` for `/rtk_fgo/odom.child_frame_id`. This keeps the optional FGO TF experiment compatible with the single-parent TF tree, while heading still represents the vehicle forward axis because `base_footprint` and `base_link` share yaw.
...
- FAST-LIO2 publishes high-rate local odometry. In the merged `frc` layout this is `odom -> base_footprint`; older bags and pre-URDF notes may show `odom -> base_link`.
...
RTK heading yaw convention: `/heading` is first decoded into the ENU yaw of the vehicle forward axis, historically written as `base_link +X`, where `0` points east and positive yaw rotates counter-clockwise toward north. In the `frc` URDF tree `base_footprint` and `base_link` share yaw, so the FGO output frame can be `base_footprint` without changing the heading convention. When heading is available, the anchor uses `map_R_enu = Rz(reference_fgo_yaw - rtk_heading_enu_yaw)`; otherwise it falls back to identity yaw and reports the lower-confidence bootstrap in diagnostics. After the anchor exists, every RTK heading factor must be rotated through the same `ENU -> map` yaw before it is compared with the FGO pose yaw.
