...
try:
    from qgis_keepout import compile_gpkg_keepout
...
def graph_component_sizes(node_count: int, edges: set[tuple[int, int]]) -> list[int]:
    adjacency = {node_id: set() for node_id in range(1, node_count + 1)}
...
def compile_qgis_scene(
    ...
    *,
    ...
    component_sizes = graph_component_sizes(len(coordinates), edges)
    if len(component_sizes) != 1:
        raise ValueError(
            "QGIS route graph is disconnected; split/snap intersections before export "
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compile QGIS route GeoJSON into nav-gps scene bundle."
    )
    parser.add_argument(
        "--input",
        required=True,
        help="QGIS GeoJSON containing feature_type=route lines.",
    )
    parser.add_argument(
        "--output",
    ...
    parser.add_argument(
        "--scene-name",
        default="qgis_scene",
        help="Scene name written to the bundle.",
    )
    parser.add_argument(
        "--densify-step-m",
        type=float,
    ...
    parser.add_argument(
        "--road-area-gpkg",
        help="Optional Polygon/MultiPolygon GeoPackage defining the drivable road area.",
    )
    parser.add_argument(
        "--road-area-layer",
        help="GeoPackage polygon layer name. Omit when the file contains one geometry layer.",
    )
    parser.add_argument(
        "--road-mask-resolution-m",
        type=float,
        default=0.10,
        help="Resolution of the generated Nav2 road keepout mask.",
    )
    parser.add_argument(
        "--road-mask-padding-m",
        type=float,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.road_area_gpkg:
        origin = bundle["nodes"][int(bundle["fixed_origin_node_id"])]
        bundle["drivable_area"] = compile_gpkg_keepout(
            args.road_area_gpkg,
            output_path.parent,
            origin_lat=float(origin["lat"]),
            origin_lon=float(origin["lon"]),
            origin_alt=float(origin.get("alt", 0.0)),
