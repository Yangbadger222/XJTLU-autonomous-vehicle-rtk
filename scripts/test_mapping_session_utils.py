...
def test_evaluate_frame_snapshot_checks_scan_and_odom_frames():
    ...
    result = evaluate_frame_snapshot(snapshot, expected_base_frame="base_link")

    assert result["ok"] is True
    assert result["scan_frame_id"] == "base_link"
    assert result["fastlio_odom_child_frame_id"] == "base_link"
