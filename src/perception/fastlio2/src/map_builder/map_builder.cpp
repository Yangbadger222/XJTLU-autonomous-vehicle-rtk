...
bool MapBuilder::process(SyncPackage &package)
{
    ...
    State state_before_prediction = m_kf->x();
    M21D covariance_before_prediction = m_kf->P();
    ...
    bool update_valid = m_lidar_processor->process(package);
    if (!update_valid)
    {
        m_kf->x() = state_before_prediction;
        m_kf->P() = covariance_before_prediction;
        return false;
    }
    return true;
}
